...
def extract_features( ppg_signal, features_params, bubble_params):
    method = features_params['method']
    r_m = bubble_params
    # # 1) Filtering the Signal
    if(method == 'simple'):
        #ppg_signalf = apply_lowpass_filter(ppg_signal, filter_params)
        features = get_features(ppg_signal, r_m)
    ...
    if(method == 'wavelet'):
        #ppg_signalf = apply_lowpass_filter(ppg_signal, filter_params)        ##wavelet with filtered signal all posible 
        #print(features_params)
        wavelet_type = features_params['type']
        level = features_params['level']  
              
        coeffs = wavedec(ppg_signal, wavelet_type, level=level) #### να αλλάξω το επίπεδο σε 2 και 3 και 4 να δούμε ότι το 5 το καλύτερο
